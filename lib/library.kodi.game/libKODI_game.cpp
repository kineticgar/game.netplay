...
DLLEXPORT void GAME_set_camera_info(AddonCB* handle, CB_GameLib* cbTable, unsigned int width, unsigned int height, GAME_CAMERA_BUFFER caps)
{
  if (handle == NULL || cbTable == NULL)
    return;

  return cbTable->SetCameraInfo(handle->addonData, width, height, caps);
}

DLLEXPORT bool GAME_start_camera(AddonCB* handle, CB_GameLib* cbTable)
{
  if (handle == NULL || cbTable == NULL)
    return false;

  return cbTable->StartCamera(handle->addonData);
}

DLLEXPORT void GAME_stop_camera(AddonCB* handle, CB_GameLib* cbTable)
{
  if (handle == NULL || cbTable == NULL)
    return;

  return cbTable->StopCamera(handle->addonData);
}

DLLEXPORT bool GAME_start_location(AddonCB* handle, CB_GameLib* cbTable)
{
  if (handle == NULL || cbTable == NULL)
    return false;

  return cbTable->StartLocation(handle->addonData);
}

DLLEXPORT void GAME_stop_location(AddonCB* handle, CB_GameLib* cbTable)
{
  if (handle == NULL || cbTable == NULL)
    return;

  return cbTable->StopLocation(handle->addonData);
}

DLLEXPORT bool GAME_get_location(AddonCB* handle, CB_GameLib* cbTable, double* lat, double* lon, double* horiz_accuracy, double* vert_accuracy)
{
  if (handle == NULL || cbTable == NULL)
    return false;

  return cbTable->GetLocation(handle->addonData, lat, lon, horiz_accuracy, vert_accuracy);
}

DLLEXPORT void GAME_set_location_interval(AddonCB* handle, CB_GameLib* cbTable, unsigned interval_ms, unsigned interval_distance)
{
  if (handle == NULL || cbTable == NULL)
    return;

  return cbTable->SetLocationInterval(handle->addonData, interval_ms, interval_distance);
}
