...
void CFrontendManager::SetCameraInfo(unsigned int width, unsigned int height, GAME_CAMERA_BUFFER caps)
{
  for (std::vector<IFrontend*>::iterator it = m_frontends.begin(); it != m_frontends.end(); ++it)
    (*it)->SetCameraInfo(width, height, caps);
}

bool CFrontendManager::StartCamera(void)
{
  bool bReturn = false;
...
void CFrontendManager::StopCamera(void)
{
  for (std::vector<IFrontend*>::iterator it = m_frontends.begin(); it != m_frontends.end(); ++it)
    (*it)->StopCamera();
}

bool CFrontendManager::StartLocation(void)
{
  bool bReturn = false;
...
void CFrontendManager::StopLocation(void)
{
  for (std::vector<IFrontend*>::iterator it = m_frontends.begin(); it != m_frontends.end(); ++it)
    (*it)->StopLocation();
}

bool CFrontendManager::GetLocation(double* lat, double* lon, double* horizAccuracy, double* vertAccuracy)
{
  for (std::vector<IFrontend*>::iterator it = m_frontends.begin(); it != m_frontends.end(); ++it)
  {
...
void CFrontendManager::SetLocationInterval(unsigned int intervalMs, unsigned int intervalDistance)
{
  for (std::vector<IFrontend*>::iterator it = m_frontends.begin(); it != m_frontends.end(); ++it)
